...
const Overview: React.FC = () => {
  ...
  const { data: allData, isLoading, error, dataUpdatedAt } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [c4tsLogs, structurizrLogs] = await Promise.all([
        fetchAllC4TSLogs(fetchStartDate, fetchEndDate), // Use the new aggregator
        fetchRawStructurizrLogsByDate(fetchStartDate, fetchEndDate), // Use the existing single-source fetcher
      ]);
  ...
  const pageData = useMemo(() => {
    ...
    const c4tsFilteredByEnv = activeFilters.environment === 'ALL' ? c4tsLogs : c4tsLogs.filter(log => log.environment === activeFilters.environment);
    const structurizrFilteredByEnv = activeFilters.environment === 'ALL' ? structurizrLogs : structurizrLogs.filter(log => log.environment === activeFilters.environment);
    ...
    // ... (The rest of the useMemo block for calculating stats and chart data remains the same as our last correct version)
